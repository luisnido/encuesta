  ...
  <div class="row-fluid">
    ...
    <div class="span10">
        ...
        <table class="table table-hover table-bordered">
            <tr><th>ID Situcion Laboral</th><th>Situacion Laboral</th> <th width='50'>Acciones</th></tr>  
            <?php if($situaciones){?>
            <?php foreach ($situaciones->result() as $situacion){
            echo '<tr><td>'.$situacion->idsituacionlaboral.'</td><td>'.$situacion->situacionlaboral.'</td><td>
                <a  data-toggle="modal"  titulo="editar" href="" id="'.$situacion->idsituacionlaboral.'"><i class="icon-edit"></i></a>&nbsp;
               <a  data-toggle="modal" titulo="detalle" href="" id="'.$situacion->idsituacionlaboral.'"><i class="icon-list"></i></a>&nbsp;
                <a  data-toggle="modal" titulo="eliminar"  href="" id="'.$situacion->idsituacionlaboral.'" ><i class="icon-remove"></i></a></td></tr>';
            }}else{}?>
</table>
...
<div class="modal-header">
    ...
    <h3 id="myModalLabel">Situacion Laboral</h3>
  </div>
    ...
    switch(titulo) {
        case 'crear':
         ...
         { $("#crearModal").modal();  
                  ...
                  $.ajax({ 
                  url:'<?php echo base_url() ?>administrador/situacion/crear/',
                  type:'get'
                  }).done(function(data) {
                 $("#crearBody").html(data);
});
        ...
        case 'editar':
         ...
         { $("#editarModal").modal();  
                  ...
                  $.ajax({ 
                  url:'<?php echo base_url() ?>administrador/situacion/editar/'+idc,
                  type:'get'
                  }).done(function(data) {
    ...
    case 'eliminar':
        $(function () 
         { $("#eliminarModal").modal();  
                  ...
                  $.ajax({ 
                  url:'<?php echo base_url() ?>administrador/situacion/eliminar/'+idc,
                  type:'get'
                  }).done(function(data) {
